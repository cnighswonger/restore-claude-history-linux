...
- **Performance/reliability:** Restore-loop peak memory drops from O(snapshots × projects × files) to O(snapshots) for the snapshot list plus O(seen-pairs) for the dedupe set. Wall-clock should be no worse than current (we do the same I/O work in a different order). No reliability regression — first-writer-wins on `created_at`-ordered snapshots gives the same observable result as pick-largest, because JSONLs are append-only (newest creation-time always implies largest size on disk).
